...
mycte3 as (
		select		teamID, yearID, cum_sum_in_billions, row_number() over(partition by teamID order by cum_sum_in_billions) as rn
		from 		mycte2
		where		cum_sum_in_billions> 1.0
)

-- select the rows with first date for each team where they crossed the $1 billion mark for spending
select 	teamID, yearID, cum_sum_in_billions
from 	mycte3
where	rn = 1